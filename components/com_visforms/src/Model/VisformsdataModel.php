...
class VisformsdataModel extends ListModel
{
		...
		if (!empty($form)) {
            ...
            foreach ($fields as $field) {
                ...
                if (empty($field->unSortable)) {
                    $xml .= '<option value="a.F' . $field->id . ' asc">' . htmlspecialchars($field->label, ENT_COMPAT, 'UTF-8') . ' ' . Text::_("COM_VISFORMS_ASC") . '</option>';
                    $xml .= '<option value="a.F' . $field->id . ' desc">' . htmlspecialchars($field->label, ENT_COMPAT, 'UTF-8') . ' ' . Text::_("COM_VISFORMS_DESC") . '</option>';
                }
            }
	...
	protected function getListBoxFilterField($field) {
		...
		$options = '<option value="">' . Text::sprintf('COM_VISFORMS_FILTER_SELECT_SELECT_A_VALUE', htmlspecialchars($field->label, ENT_COMPAT, 'UTF-8')) . '</option>';
	...
	protected function getCheckboxFilterField($field) {
		...
		$options .= '<option value="unchecked">' .Text::_('COM_VISFORMS_NOT')  . ' '. htmlspecialchars($field->attribute_value, ENT_COMPAT, 'UTF-8') . '</option>';
		$xmlstring = '<field
			name="' . $this->paginationcontext . $field->name . '"
			type="list"
			class="form-control"
