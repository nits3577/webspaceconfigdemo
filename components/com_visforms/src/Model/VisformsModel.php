...
class VisformsModel extends BaseDatabaseModel
{
	...
	public function getForm() {
		...
		if (empty($storedFormIsValid)) {
			...
			//set a flag, if we use a bootstrap 3 Layout used in view file in tmpl folder
            // ToDo If there are layout problems with bootstrap 3 in edit views add 'editbt3default','editbt3horizontal', 'editbt3mcindividual' to the list
			$this->form->hasBt3Layout = (in_array($this->form->formlayout, array('bt3default', 'bt3mcindividual', 'bt3horizontal'))) ? true : false;
			...
			$this->form->canHideSummaryOnMultiPageForms = (empty($this->aefList[\VisformsAEF::$subscription])) ? false : true;
			$this->form->mpdisplaytype = (empty($this->form->mpdisplaytype) || empty($this->aefList[\VisformsAEF::$subscription])) ? 0 : $form->mpdisplaytype;
			...
			if (!empty($this->form->captcha) && ($this->form->captcha == "2") && (!PluginHelper::isEnabled('captcha', 'recaptcha'))) {
				$this->form->captcha = "0";
			}
	...
	public function reloadFields() {
		...
		$visform->canHideSummaryOnMultiPageForms = (empty($this->aefList[\VisformsAEF::$subscription])) ? false : true;
		$visform->mpdisplaytype = (empty($visform->mpdisplaytype) || empty($this->aefList[\VisformsAEF::$subscription])) ? 0 : $visform->mpdisplaytype;
		$app = Factory::getApplication();
		$this->fields = $app->getUserState('com_visforms.' . $this->context . '.fields');
		if (!is_array($this->fields)) {
	...
	private function storeData($visform) {
		...
		$datas = new \stdClass();
		$datas->created = Factory::getDate()->toSql();
	...
	protected function sendReceiptMail($visform) {
		...
		if (!empty($visform->emailreceipttext)) {
			//Do some replacements in email text
			$replacedText = HTMLHelper::_('visforms.replacePlaceholder', $visform, $visform->emailreceipttext);
		...
		try {
			...
			$emailreceiptsubject = HTMLHelper::_('visforms.replacePlaceholder', $visform, $visform->emailreceiptsubject);
			...
			// the receipt mail could not be send. Give a warning.
			$app->enqueueMessage(Text::sprintf('COM_VISFORMS_PROBLEMS_SENDING_MAIL', Text::_('COM_VISFORMS_RECEIPT_MAIL')) . $e->getMessage(), 'warning');
		}
	}
	...
	protected function sendResultMail($visform) {
		...
		if ((!empty($visform->emailresulttext))) {
			$replacedText = HTMLHelper::_('visforms.replacePlaceholder', $visform, $visform->emailresulttext);
			$mailBody .= HTMLHelper::_('visforms.fixLinksInMail', $replacedText);
		}
		$bodyData = array();
		...
		try {
			...
			$subject = HTMLHelper::_('visforms.replacePlaceholder', $visform, $visform->subject);
			...
			if (empty($visform->saveresult)) {
				// form data are not stored in the database and result mail failed. So data are completely lost. Throw error.
				throw new \RuntimeException(Text::sprintf('COM_VISFORMS_PROBLEMS_SENDING_MAIL', Text::_('COM_VISFORMS_RESULT_MAIL')) . $e->getMessage());
			}
			else {
				// form data are stored successfully in the database but result mail failed. Give a warning.
				$app->enqueueMessage(Text::sprintf('COM_VISFORMS_PROBLEMS_SENDING_MAIL', Text::_('COM_VISFORMS_RESULT_MAIL')) . $e->getMessage(), 'warning');
			}
		}
	}
	...
	protected function getAttachementName($visform , $atmName, $uploadName) {
		...
		$attachmentName = HTMLHelper::_('visforms.replacePlaceholder', $visform, $atmName) . '.' . $fileExt;
