    ...
    <script type="text/javascript">
        jQuery(document).ready(function () {
                jQuery('#<?php echo $parentFormId; ?>').validate({
                    ...
                    messages: { <?php
                        // ToDo replace all possible occurances of @ in error message, because Joomla email cloaking will break javascript
                        // Add field type specific Visforms error messages (Language tags can be modified, using Joomla language manager).
						for ($i = 0; $i < $nbFields; $i++) {
                            $field = $visforms->fields[$i];
                            // Field type specific Visforms error messages for date fields
							if (isset($field->typefield) && $field->typefield == "date" && !(isset($field->customErrorMsgArray))) {
								if (isset($field->dateFormatJs)) {
									...
									switch ($field->dateFormatJs) {
                                        case "%d.%m.%Y":
                                             echo " dateDMY: \"" . Text::sprintf('COM_VISFORMS_ENTER_VALID_DATE_FORMAT', 'dd.mm.YYYY') . "\", ";
                                            break;
                                        case "%m/%d/%Y":
                                            echo " dateMDY: \"" . Text::sprintf('COM_VISFORMS_ENTER_VALID_DATE_FORMAT', 'mm/dd/YYYY') . "\", ";
                                            break;
                                        case "%Y-%m-%d":
                                            echo " dateYMD: \"" . Text::sprintf('COM_VISFORMS_ENTER_VALID_DATE_FORMAT', 'YYYY-mm-dd') . "\", ";
                                            break;
                                    }
	                                echo " },";
                                }
                                ...
                                echo "\"" . $field->name . "\" : { filesize: \"" . Text::sprintf('COM_VISFORMS_JS_ERROR_WARNFILETOOLARGE', ($visforms->maxfilesize)) . "\" ,";
                                echo " fileextension: \"" . Text::_('COM_VISFORMS_JS_ERROR_WARNFILETYPE') . "\" },";
                            }
	                            ...
	                            echo "\"" . $field->name . "_code\" : { remote: \"" . Text::_('COM_VISFORMS_VALIDATION_CODE_INVALID') . "\",";
	                            echo " required : \"". Text::_('COM_VISFORMS_ENTER_VALIDATION_CODE') ."\"} ,";
                            }
                            // User defined error message (from field configuration, @ allready replaced)
							if (isset($field->customErrorMsgArray)) {
                                ...
                                if (isset($field->typefield) && ($field->typefield == "select" || $field->typefield == "selectsql" || $field->typefield == "multicheckbox" || $field->typefield == "multicheckboxsql")) {
                                    ...
                                    // User defined error message for 'normal' fields
                                    echo "\"" . $field->name . "\": {";
									foreach ($field->customErrorMsgArray as $n => $v) {
								...
								if (isset($field->typefield) && (($field->typefield == "multicheckbox") || ($field->typefield == "multicheckboxsql"))) {
                                    echo "\"" . $field->name . "[]\": {";
                                    echo "minlength: jQuery.validator.format('" . Text::_('COM_VISFORMS_ENTER_VAILD_MINLENGTH_MULTICHECKBOX') . "'),";
                                    echo "maxlength: jQuery.validator.format('" . Text::_('COM_VISFORMS_ENTER_VAILD_MAXLENGTH_MULTICHECKBOX') . "')";
                                    echo "},";
                                }
                            }
                        }
                        // Custom Captcha Error Message (ToDo escape potential @)
                    ...
                    <?php if (!empty($visforms->mpdisplaytype)) { ?>
                        ...
                        showErrors: function(errorMap, errorList) {
                            ...
                            errorNoteDiv.html("<?php echo Text::_('COM_VISFORMS_VALIDATOR_ERROR_COUNT_MESSAGE1'); ?>"
                                + this.numberOfInvalids()
                                + "<?php echo Text::_('COM_VISFORMS_VALIDATOR_ERROR_COUNT_MESSAGE2'); ?>");
