		...
		if (!empty($link)) {
			if (!empty($displayId)) {
				if (!empty($addLinkAsText)) {
					...
					echo '<' . $htmlTag . $class . '><a class="' . $linkClass . '" href="' . $link . '" title="' . Text::_('COM_VISFORMS_VIEW_DETAIL') . '"' . $dataAttrib . ' data-bs-toggle="tooltip">' . Text::_('COM_VISFORMS_VIEW_DETAIL') . '</a></' . $htmlTag . '>';
				}
				else {
					echo '<' . $htmlTag . $class . '>'. $hiddenCheckbox . '<a class="' . $linkClass . '" href="' . $link . '" title="' . Text::_('COM_VISFORMS_VIEW_DETAIL') . '"' . $dataAttrib . ' data-bs-toggle="tooltip">' . $data->id . '</a></' . $htmlTag . '>';
				}
			}
			else {
				if (!empty($addLinkAsText)) {
					echo '<' . $htmlTag . $class . '>'. $hiddenCheckbox . '<a class="' . $linkClass . '" href="' . $link . '" title="' . Text::_('COM_VISFORMS_VIEW_DETAIL') . '"' . $dataAttrib . ' data-bs-toggle="tooltip">' . Text::_('COM_VISFORMS_VIEW_DETAIL') . '</a></' . $htmlTag . '>';
				}
				else {
					$detailicon = (!empty($form->detaillinkitem)) ? $form->detaillinkitem : 'download';
					echo '<' . $htmlTag . $class . '>'. $hiddenCheckbox . '<a class="' . $linkClass . '" href="' . $link . '" title="' . Text::_('COM_VISFORMS_VIEW_DETAIL') . '"' . $dataAttrib . ' data-bs-toggle="tooltip"><i class="visicon-' . $detailicon . '"></i></a></' . $htmlTag . '>';
				}
			}
		}
		else {
