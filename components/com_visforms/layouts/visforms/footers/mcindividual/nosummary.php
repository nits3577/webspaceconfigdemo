	...
	if (isset($displayData['form']) && isset($displayData['nbFields']) && isset($displayData['hasRequired'])) :
		...
		if (isset($form->captcha) && ($form->captcha == 1 || $form->captcha == 2)) {
			if ($form->formlayout == 'bt3mcindividual') {
				echo LayoutHelper::render('visforms.captcha.bt3mcindividual', array('form' => $form), null, array('component' => 'com_visforms'));
			}
			else {
				echo LayoutHelper::render('visforms.captcha.mcindividual', array('form' => $form), null, array('component' => 'com_visforms'));
			}
		}
