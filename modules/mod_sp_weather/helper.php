    ...
    */
    ...
    private function _getWeatherData($type = 'current') {
        if($type == 'forecast') {
            ...
            if ($this->platform == 'apixu') {
                $this->api = 'https://api.apixu.com/v1/forecast.json?q='. $this->location .'&days='. $this->forecast_limit .'&key=' . $this->api_key;
            } elseif ($this->platform == 'weatherbit') {
                ...
                if($this->getdataby == 'locaion_id') {
                    $this->api  = 'https://api.openweathermap.org/data/2.5/forecast/daily?id='. $this->locationid .'&units=metric&cnt='. $this->forecast_limit .'&lang=en&appid=' . $this->api_key;
                } else {
                    $this->api  = 'https://api.openweathermap.org/data/2.5/forecast/daily?q='. $this->location .'&units=metric&cnt='. $this->forecast_limit .'&lang=en&appid=' . $this->api_key;
                }
            }
        } else {
            if ($this->platform == 'apixu') {
                $this->api  = 'https://api.apixu.com/v1/current.json?q='. $this->location .'&key=' . $this->api_key;
            } elseif ($this->platform == 'weatherbit') {
                if($this->getdataby == 'latlon') {
                    $location_latlon = explode(',', str_replace(', ', ',', $this->location_latlon));
                ...
                if($this->getdataby == 'locaion_id') {
                    $this->api       = 'https://api.openweathermap.org/data/2.5/weather?id='. $this->locationid .'&units=metric&&appid=' . $this->api_key;
                } else {
