...
class VisdatasModel extends ListModel
{
	public $_data = Array();
    ...
    public function getFilterForm($data = array(), $loadData = true) {
        ...
        foreach($datafields as $dataField) {
            if(isset($dataField->showFieldInDataView) && $dataField->showFieldInDataView == true && empty($dataField->unSortable)) {
                $xml .= '<option value="a.F' . $dataField->id . ' ASC">' . $dataField->name . ' ' . Text::_("COM_VISFORMS_ASC") . '</option>';
                $xml .= '<option value="a.F' . $dataField->id . ' DESC">' . $dataField->name . ' ' . Text::_("COM_VISFORMS_DESC") . '</option>';
            }
        }
        ...
        // field is always appened at the end of a group: in order to keep the limit field at the end, we have to add it, too
        $xml = '<field
			name="limit"
			type="limitbox"
