...
{
	protected $aefList;
	protected $allowedCalculationPlaceholderFieldTypes = array('number', 'select', 'calculation', 'hidden', 'checkbox', 'text', 'date', 'radio');
	...
	public function save($data) {
		...
		if ($task != 'save2copy') {
			...
			if (empty($this->aefList[\VisformsAEF::$subscription]) || $data['typefield'] !== 'location' || empty($data['frontdisplay'])) {
		...
		if (isset($data['defaultvalue']) && is_array($data['defaultvalue'])) {
            ...
            if (in_array($data['typefield'], array('text', 'email', 'hidden', 'number', 'url', 'date'))) {
                // clean data
                if (empty($this->aefList[\VisformsAEF::$subscription])) {
                    if ($data['defaultvalue']['f_'.$data['typefield'].'_fillwith'] == 'sql') {
                        $data['defaultvalue']['f_'.$data['typefield'].'_fillwith'] = "0";
                        // email uses different option value for fillwith Startvalue (inival instead of 0 like the other fieldtypes
                        // ToDo clean code. Provide new demoform.json
                        if ($data['typefield'] == 'email') {
                            $data['defaultvalue']['f_'.$data['typefield'].'_fillwith'] = "inival";
                        }
                    }
                }
                if ($data['defaultvalue']['f_'.$data['typefield'].'_fillwith'] !== 'sql') {
            ...
            if ($task != 'save2copy') {
                ...
                if (!empty($this->aefList[\VisformsAEF::$subscription]) && $data['typefield'] === 'location') {
                ...
                if ($data['typefield'] === "date" && $data['defaultvalue']['f_date_attribute_value'] === Factory::getDbo()->getNullDate()) {
	            ...
	            if (!empty($data['id'])) {
		            ...
		            if (!$this->user->authorise('core.create.sql.statement', 'com_visforms') || empty($this->aefList[\VisformsAEF::$subscription])) {
	...
	public function getForm($data = array(), $loadData = true) {
		...
		if ($fid != 0) {
			// form configuration dependend settings
			$model = new VisformModel(array('ignore_request' => true));
			//JModelLegacy::getInstance('Visform', 'VisformsModel', array('ignore_request' => true));
			...
			// unsupported feature in bt2 and 3 and default
			if (empty($this->aefList[\VisformsAEF::$subscription]) || (!empty($visform) && (!in_array($visform->layoutsettings['formlayout'], array('bt5', 'bt4mcindividual', 'uikit3', 'uikit2'))))) {
			...
			if (empty($this->aefList[\VisformsAEF::$subscription]) || ((!empty($visform) && (!isset($visform->layoutsettings['displaysublayout']) || ($visform->layoutsettings['displaysublayout'] != 'individual'))))) {
				$form->removeGroup('gridSizes');
			}
			// 4 breakpoints in uikit 2
			...
			// 5 breakpoints in bt4 and uikitt 3
            if (!empty($visform)  && in_array($visform->layoutsettings['formlayout'], array('uikit3', 'bt4mcindividual')) && isset($visform->layoutsettings['displaysublayout']) && $visform->layoutsettings['displaysublayout'] == 'individual') {
                $form->removeField('fieldsPerRowXxl', 'gridSizes');
                $form->removeField('labelBootstrapWidthXxl', 'gridSizes');
            }
		}
        ...
        if (empty($this->aefList[\VisformsAEF::$subscription])) {
            ...
            $form->removeField('f_text_defaultvalue_sql', 'defaultvalue');
            $form->removeField('f_email_defaultvalue_sql', 'defaultvalue');
            $form->removeField('f_hidden_defaultvalue_sql', 'defaultvalue');
            $form->removeField('f_number_defaultvalue_sql', 'defaultvalue');
            $form->removeField('f_url_defaultvalue_sql', 'defaultvalue');
            $form->removeField('f_date_defaultvalue_sql', 'defaultvalue');
    ...
    protected function getReorderConditions($table) {
	    return 'fid = '.Factory::getApplication()->input->get('fid', 0);
	}
