...
class Visforms
{
	...
	public static function creditsBackend() {
		$html = '<div class="row-fluid"><div class="visformbottom col-lg-12" style="padding-top: 15px; padding-bottom:19px; color: #999; text-align: center;margin-top:40px;">Visforms ' . Text::_('COM_VISFORMS_VERSION') . ' ' . self::getVersion() . (!empty(\VisformsAEF::getVersion(12)) ? ', Subscription ' . Text::_('COM_VISFORMS_VERSION') . ' ' . \VisformsAEF::getVersion(12): ' ' . Text::_('COM_VISFORMS_WITHOUT_SUBSCRIPTION')) . ', &copy; 2012 - ' . self::getCopyRightDate() . ' <a href="http://vi-solutions.de" target="_blank" class="smallgrey">vi-solutions</a>' . Text::_('COM_VISFORMS_OPEN_SOFTWARE') . ' <a href="http://www.gnu.org/licenses/gpl-2.0.html" target="_blank" class="smallgrey">GNU/GPL License</a>.</div></div>';
		return $html;
	}
	...
	public static function getVersion() {
		$xml_file = Path::clean(JPATH_ADMINISTRATOR . '/components/com_visforms/visforms.xml');
	...
	public static function getCopyRightDate() {
		$xml_file = Path::clean(JPATH_ADMINISTRATOR . DIRECTORY_SEPARATOR . 'components' . DIRECTORY_SEPARATOR . 'com_visforms' . DIRECTORY_SEPARATOR . 'visforms.xml');
		$crd = HTMLHelper::_('date', 'now', 'Y');
		if (file_exists($xml_file)) {
			//supress warnings
			libxml_use_internal_errors(true);
			$xml = simplexml_load_file($xml_file);
	...
	public static function getUploadFileFullPath($registryString) {
		...
		if (isset($fileInfo['folder'])) {
			...
			return Uri::root() . $registryString;
		}
	}
	...
	public static function getUploadFileLink($registryString) {
		...
		if (isset($fileInfo['folder']) && isset($fileInfo['file'])) {
			// return link
			return '<a href="' . Uri::root() . $fileInfo['folder'] . '/' . $fileInfo['file'] . '" target="_blank">' . Uri::root() . $fileInfo['folder'] . '/' . $fileInfo['file'] . '</a>';
		}
		else {
	...
	public static function includeScriptsOnlyOnce($cssScripts = array('visforms.default.min' => true, 'bootstrapform' => false), $jsScripts = array('validation' => true)) {
		...
		foreach ($cssScripts as $scriptName => $scriptValue) {
			if (empty(static::$loaded['cssFile'][$scriptName]) && $scriptValue) {
				$doc->addStyleSheet(URI::root(true) . '/media/com_visforms/css/' . $scriptName . '.css', array('version' => 'auto', 'relative' => false, 'detectBrowser' => false, 'detectDebug' => false));
		...
		if ($jsScripts['validation'] == true) {
			...
			HTMLHelper::_('script', 'media/com_visforms/js/jquery.validate.min.js', array('version' => 'auto', 'relative' => false, 'detectBrowser' => false, 'detectDebug' => false));
			HTMLHelper::_('script', 'media/com_visforms/js/visforms.js', array('version' => 'auto', 'relative' => false, 'detectBrowser' => false, 'detectDebug' => false));
	...
	public static function sort($title, $order, $direction = 'asc', $selected = '', $task = null, $new_direction = 'asc', $tip = '', $form = 'adminForm', $jsFunction = "Joomla.tableOrdering", $unSortable = false) {
		...
		$html[] = '<a href="#" onclick="' . $jsFunction . '(\'' . $order . '\',\'' . $direction . '\',\'' . $task . '\',document.getElementById(\'' . $form . '\'));return false;"'
			. ' class="visToolTip" title="' . Text::_('JGLOBAL_CLICK_TO_SORT_THIS_COLUMN') . '" data-bs-toogle="tooltip">';
	...
	public static function visformsTooltip() {
        ...
        Factory::getApplication()->getDocument()->addStyleSheet(URI::root(true) . '/media/com_visforms/css/visforms.tooltip.min.css', array('version' => 'auto', 'relative' => false, 'detectBrowser' => false, 'detectDebug' => false));
	...
	public static function addDetailAdminForm() {
		...
		$script = 'window.addDetailAdminForm = function (adminformidprefix, actionurl, returnurl, id) {
	    	...
	    	\'<input class="btn" type="submit" value="' . Text::_("COM_VISFORMS_DOWNLOAD_PDF") . '"/>\' +
			\'<input type="hidden" name="task" value="visformsdata.renderPdf"/>\' +
			\'<input type="hidden" name="return" value="\' + returnurl + \'" />\' +
			\'<input type="hidden" name="cid[]" value="\' + id + \'" />' . HTMLHelper::_('form.token') . '\' +
	    	\'</form>\';
