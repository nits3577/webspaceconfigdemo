...
class visformsSql
{
	...
	public function getItemsFromSQL($queryFunction = 'loadObjectList') {
	    // filter $queryFunction parameter. Only allow the following Joomla database query functions, which process select statements
	    $allowedQueryFunctions = array('loadObjectList', 'loadResult');
	    if (!in_array($queryFunction, $allowedQueryFunctions, true)) {
            $queryFunction = 'loadObjectList';
        }
		...
		if ($this->checkIsSelectSqlOnly()) {
			...
			throw  new Exception(Text::_("COM_VISFORMS_CREATE_SQL_BAD_SQL"));
		}
	}

	// ToDo use different implementations for user and input object
	private function processFrameworkObjects($text) {
		// replace placeholder for framework objects with value
	...
	protected function parseSql() {
		...
		try {
			// calcPosition throws errors really fast and it's use it not necessary
