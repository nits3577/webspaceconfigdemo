            ...
            jQuery.ajax({
                ...
                success: function(data, textStatus, jqXHR) {
                    ...
                    if (data.success) {
                        ...
                        jQuery('#myTab a').not("[aria-selected='true']").hide();
                        jQuery('#myTab li').last().find('a').show();
                        ...
                        var title = '{$fntext('COM_VISFORMS_CREATOR_FIELD_TABLE_OPEN_DESC')}';
                        jQuery(tab).find('tr th:nth-last-child(1)').text('{$fntext('COM_VISFORMS_CREATOR_FIELD_TABLE_OPEN')}');
                            ...
                            if (typeof data.fields[index] !== 'undefined') {
                                ...
                                if ('gender' !== name && 'sex' !== name) {
                                    if ('multicheckbox' === type || 'select' === type || 'radio' === type) {
                                        style = 'color: #FF5722;'
                                        title = '{$fntext('COM_VISFORMS_CREATOR_FIELD_TABLE_OPEN_SELECT_DESC')}';
                                    }
                                }
        ...
        <div class="<?php if (!empty($this->sidebar)) {echo 'col-md-9 col-xl-10'; } else { echo 'col-12'; } ?>">
            ...
            <div class="form-horizontal"><?php
                ...
                <div class="row-fluid btns-create-form">
                    <button data-bs-content="<?php echo Text::_('COM_VISFORMS_CREATOR_CREATE_DATA'); ?>" onclick="visHelper.createExampleData(event)" class="btn btn-small btn-info btn-create-example-data hasPopover" disabled><?php echo Text::_('COM_VISFORMS_CREATOR_CREATE_DATA'); ?></button>
